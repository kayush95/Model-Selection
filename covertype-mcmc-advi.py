...
model_mcmc = make_nn(X_downsamp, Y_downsamp, n_hidden=n_hidden)

with model_mcmc:
    approx_mcmc = pm.fit(200000,
                    callbacks=[pm.callbacks.CheckParametersConvergence(tolerance=1e-1)])
...
with model_mcmc:
    trace_mcmc = approx.sample(1000)

with model_mcmc:
    samp_ppc_mcmc = pm.sample_ppc(trace_mcmc[100:], samples=1000)

preds_proba = samp_ppc['likelihood'].mean(axis=0)
preds = (preds_proba == np.max(preds_proba, axis=0, keepdims=True)) * 1
print('Accuracy on data for ADVI= {}%'.format(accuracy_score(Y_downsamp.transpose().values, preds) * 100))

preds_proba_mcmc = samp_ppc['likelihood'].mean(axis=0)
preds_mcmc = (preds_proba_mcmc == np.max(preds_proba_mcmc, axis=0, keepdims=True)) * 1
print('Accuracy on data for MCMC= {}%'.format(accuracy_score(Y_downsamp.transpose().values, preds_mcmc) * 100))

waic = pm.waic(trace[100:], neural_network)
loo = pm.loo(trace[100:], neural_network)

print(f"WAIC for ADVI-trained network: {waic.WAIC}")
print(f"LOO for ADVI-trained network: {loo.LOO}")

waic_mcmc = pm.waic(trace_mcmc[100:], neural_network)
loo_mcmc = pm.loo(trace_mcmc[100:], neural_network)

print(f"WAIC for MCMC-trained network: {waic_mcmc.WAIC}")
print(f"LOO for MCMC-trained network: {loo_mcmc.LOO}")